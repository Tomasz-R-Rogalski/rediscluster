            ...
            steps {
                script {
                   ...
                   //     sh "kubectl get pods -l app=redis-cluster -o jsonpath=\'{range.items[*]}{.status.podIP}\'"
                        sh "kubectl exec -i redis-cluster-0 -- redis-cli --cluster create --cluster-replicas 1 \$(kubectl get pods -l app=redis-cluster -o jsonpath='{range.items[*]}{.status.podIP}:6379 ' | sed 's/6379 :6379/6379/') -auto-approve"
                    //    sh "kubectl exec -i redis-cluster-0 -- redis-cli --cluster create --cluster-replicas 1 \$(kubectl get pods -l app=redis-cluster -o jsonpath='{range.items[*]}{.status.podIP}:6379 ' | )"
